...
const EIP712_1 = require("../EIP712/EIP712");
const Options_1 = require("../Options/Options");
const EIP712StructTypes_1 = require("./helpers/EIP712StructTypes");
...
class ExportFCT {
    ...
    getTypedDataTypes() {
        ...
        if (Number(recurrency.maxRepeats) > 1) {
            optionalTypes = lodash_1.default.merge(optionalTypes, { Recurrency: EIP712_1.EIP712.recurrency });
            additionalTypesInPrimary.push({ name: "recurrency", type: "Recurrency" });
        }
        if (multisig.externalSigners.length > 0) {
            optionalTypes = lodash_1.default.merge(optionalTypes, { Multisig: EIP712_1.EIP712.multisig });
            additionalTypesInPrimary.push({ name: "multisig", type: "Multisig" });
        }
        if (this.FCT.computed.length > 0) {
            optionalTypes = lodash_1.default.merge(optionalTypes, { Computed: EIP712_1.EIP712.computed });
        }
        return {
            EIP712Domain: EIP712_1.EIP712.domain,
            Meta: EIP712_1.EIP712.meta,
            Limits: EIP712_1.EIP712.limits,
            ...optionalTypes,
            ...transactionTypes,
            ...structTypes,
            BatchMultiSigCall: this.getPrimaryTypeTypes(additionalTypesInPrimary),
            Call: EIP712_1.EIP712.call,
        };
    }
    getTypedDataDomain() {
    ...
    getTransactionTypedDataMessage() {
        return this.calls.reduce((acc, call, index) => {
            let paramsData = {};
            if (call.params) {
                paramsData = getParams(call.params);
            }
