    ...
    const getValues = (param) => {
        ...
        if (param.hashed) {
            if (typeof param.value === "string") {
                return ethers.utils.keccak256(toUtf8Bytes(param.value));
            }
            throw new Error("Hashed value must be a string");
        }
        return param.value;
    ...
    getTransactionTypedDataMessage() {
        return this.FCT.decodedCalls.reduce((acc, call, index) => {
            ...
            if (call.multicall) {
                ...
                paramsData = call.params ? getParams(call.params) : {};
            }
...
const isAddress$1 = ethers.ethers.utils.isAddress;
...
class ExportFCT extends FCTBase {
    ...
    get() {
        return {
            ...
            typeHash: hexlify(ethSigUtil.TypedDataUtils.hashType(this.typedData.primaryType, this.typedData.types)),
        ...
        return calls.map((call, index) => {
            ...
            return {
                ...
                typedHashes: usedTypeStructs.length > 0
                    ? usedTypeStructs.map((hash) => hexlify(ethSigUtil.TypedDataUtils.hashType(hash, typedData.types)))
                    : [],
            };
        });
    }
...
class FCTCalls extends FCTBase {
    ...
    async create(call) {
        if (InstanceOf.IWithPlugin(call)) {
            ...
            return this.createMulticall(call);
        }
        else {
...
class FCTUtils extends FCTBase {
    ...
    getMessageHash() {
        return ethers.ethers.utils.hexlify(ethSigUtil.TypedDataUtils.eip712Hash(this.FCTData.typedData, ethSigUtil.SignTypedDataVersion.V4));
    }
...
const getMulticallContract = (chainId, provider) => {
    ...
    return new ethers.ethers.Contract(multicallAddress, [
        "function aggregate((address target, bytes callData)[] calls) external view returns (uint256 blockNumber, bytes[] returnData)",
    ], provider);
};
const getData = async ({ chainId, provider }) => {
    ...
    const UniswapV2Pair = new ethers.ethers.utils.Interface([
    ...
    const [price0CumulativeLast, price1CumulativeLast, getReserves, s_blockTimestampLast, s_price0CumulativeLast, s_price1CumulativeLast, s_timeBetweenKiroPriceUpdate, s_price0Average, s_price1Average,] = returnData.map((data, i) => {
        ...
        return ethers.ethers.BigNumber.from(data);
    });
    return {
        blockNumber,
