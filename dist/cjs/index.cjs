        ...
        return allPayers.map((payer) => {
            ...
            return {
                ...
                largestPayment: {
                    ...
                    tokenAmount: ethers.utils.formatEther(largest.kiroCost.toString()),
                    nativeAmount: ethers.utils.formatEther(largest.ethCost.toString()),
                },
                smallestPayment: {
                    gas: smallest.gas.toString(),
                    tokenAmountInWei: smallest.kiroCost.toString(),
                    nativeAmountInWei: smallest.ethCost.toString(),
                    tokenAmount: ethers.utils.formatEther(smallest.kiroCost.toString()),
                    nativeAmount: ethers.utils.formatEther(smallest.ethCost.toString()),
                },
            };
        });
    };
...
function exportFCT() {
    ...
    return {
        ...
        appHash: utils$1.id(this.options.app.name),
        appVersionHash: utils$1.id(this.options.app.version),
        builderHash: utils$1.id(this.options.builder.name),
        builderAddress: this.options.builder.address,
        domainHash: utils$1.id(this.options.domain),
        verifierHash: utils$1.id(this.options.verifier),
...
async function exportWithApprovals() {
    ...
    for (const signer of signers) {
        ...
        const ERC20Approvals = new fctPlugins.Erc20Approvals({
            chainId: FCT.chainId,
        });
        const ResetERC20Approvals = new fctPlugins.Erc20Approvals({
            chainId: FCT.chainId,
        });
...
async function getPlugin(index) {
    ...
    if (callData.toENS === "@lib:multicall") {
        const plugin = fctPlugins.getMulticallPlugin({
