...
contract FactoryProxy_ is FactoryStorage {
    ...
    function uid() external pure returns (bytes32) { return 0x00; }
    function activator() external pure returns (address) { return address(0); }
    // Stubs ends
    ...
    bytes32 public constant BATCH_MULTI_SIG_CALL_TRANSACTION_TYPEHASH_ =
        keccak256(
            "Transaction_(address signer,address call_address,string call_ens,uint256 eth_value,uint32 gas_limit,bool view_only,bool continue_on_fail,bool stop_on_fail,bool stop_on_success,bool revert_on_success,string method_interface)"
        );
    ...
    function setLocalEns_(string calldata ens, address dest) external onlyOwner {
            ...
            for (uint256 i = 0; i < tr.length; i++) {
                ...
                if (!success) {
                    if (!localSilentRevert) {
                        emit TransferPackedReverted_(wallet.addr, localNonce, i);
                        continue;
                    } else {
                        revert(_getRevertMsg(res));
                    }
    ...
    function batchMultiCall_(MCalls[] calldata tr, uint256 nonceGroup) external {
        require(msg.sender == s_activator, "Wallet: sender not allowed");
        unchecked {
            uint256 ng = nonceGroup;
            uint256 nonce = s_nonce_group[ng] + (ng << GROUP_BIT);
            ...
            s_nonce_group[ng] = _nextNonce(nonce, maxNonce);
            emit BatchTransfered_(2, block.number, maxNonce);
        }
    }
    ...
    function batchMultiSigCall_(MSCalls[] calldata tr, uint256 nonceGroup)
        external
    {
        require(msg.sender == s_activator, "Wallet: sender not allowed");
        unchecked {
            ...
            s_nonce_group[ng] = _nextNonce(nonce, maxNonce);
            emit BatchTransfered_(3, block.number, maxNonce);
        }
    }
