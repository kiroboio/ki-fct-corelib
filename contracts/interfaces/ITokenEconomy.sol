...
interface IActivator {
  ...
  event InheritanceFees(
    uint256 minFees
  );

  event TokenInheritanceFees(
    uint256 minFees
  );
...
interface IInheritancePayment {
  ...
  function migrateTokenInheritance(address vaultOwner, address from, address[] calldata tokens) external;
  function updateTokenInheritanceFunds(address vaultOwner, address[] calldata tokensToAdd, address[] calldata tokensToRemove, uint256 fundsPerToken) external;
  function activateTokenInheritance(address vault, address token) external;

  function updateAllInheritanceFunds(address vaultOwner, uint256 funds, address[] calldata tokensToAdd, address[] calldata tokensToRemove, uint256 fundsPerToken) external;
  function updateAllFunds(address vaultOwner, uint256 backupFunds, uint256 inheritanceFunds, address[] calldata tokensToAdd, address[] calldata tokensToRemove, uint256 fundsPerToken) external;
}
