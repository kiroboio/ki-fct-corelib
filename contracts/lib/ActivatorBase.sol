...
abstract contract ActivatorBase is IActivator, AccessControl {
  ...
  @param timeout the time needed to pass untill an activator can again activate a transaction*/
    function setTimeout(uint256 timeout)
        external
        override
    ...
    function setRequireFunds(bool requireFunds)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        s_requireFunds = requireFunds;
        emit RequireFundsChanged(requireFunds);
    }

    function getRequireFunds() external view override returns (bool) {
        return s_requireFunds;
    }
  ...
  @param createFee amount of fee*/
    function setInheritanceFees(uint256 createFee)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
  ...
  @param createFee amount of fee*/
    function setTokenInheritanceFees(uint256 createFee)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
  ...
  @param createFee amount of fee*/
    function setBackupFees(uint256 createFee)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
  ...
  @param discountNFT NFT address, that gives a discount for holding kiro as an activator*/
    ...
    function getBackupFees() external view override returns (Fees memory) {
    ...
    function getInheritanceFees() external view override returns (Fees memory) {
