...
contract RecoveryWallet is IMigrate, StorageBase, Storage, ReentrancyGuard, Interface {
  ...
  function execute(
    ...
    require(to != this.creator(), "direct factory call");
  ...
  function execute2(address payable to, uint256 value, bytes calldata data) external onlyActiveState onlyGasReturnContract(msg.sender) returns (bytes memory){
    require(to != this.creator(), "direct factory call");
    (bool success, bytes memory res) = to.call{value: value}(data);
    if(!success){
        revert(_getRevertMsg((res)));
