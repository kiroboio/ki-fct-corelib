...
contract GasReturn is AccessControl {
  ...
  //uint256 public s_kiroForEthInWei;
  uint256 private s_stakingAmountNeeded;
  //address private s_kiroEthPairAddress;
...
// --------------------------------------------
  ...
  mapping(uint256 => uint256) private s_idToCollectStatus;
  ...
  constructor(address activator, address factory, address kiroAddress, address i_pair) {
    ...
    IUniswapV2Pair _pair = IUniswapV2Pair(i_pair);
    pair = _pair;
    token0 = _pair.token0();
    token1 = _pair.token1();
  ...
  function updateKiroPrice() public {
  ...
  */
  ...
  /**@dev gets how much a specific ndt id has left to use in gas
  ...
  /** @dev calculate price based on pair reserves*/
  /* function getKiroTokenPrice() public view returns (uint256 kiroForEthInWei) {
    IUniswapV2Pair pair = IUniswapV2Pair(s_kiroEthPairAddress);
    (uint256 kiro, uint256 ETH, ) = pair.getReserves();
    return kiroForEthInWei = kiro / ETH;
  } */
  ...
  /**@dev gets the current system kiro price */
  /* function getKiroPrice() external view returns (uint256 kiroPrice) {
    kiroPrice = s_kiroForEthInWei;
  } */
...
@param input array of nft values (ie: [100000, 50000, 20000, 10000, 5000, 2500, 2000]) */
  ...
  /**@dev returns the status of the collect 
    @param tokenId nft id*/
  function getCollectStatus(uint256 tokenId) external view returns (uint256) {
    return s_idToCollectStatus[tokenId];
  }
  ...
  closing a month means to get the gane resolts for the closed 
  month and award the NDT with the prize earnings
  ...
  /**@dev updates s_kiroForEthInWei and s_lastUpdateDateOfPrice */
  /*function updateKiroPrice() internal {
    s_kiroForEthInWei = getKiroTokenPrice();
    s_lastUpdateDateOfPrice = block.timestamp;
  } */
