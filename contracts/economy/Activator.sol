...
contract Activator is IBackupPayment, IInheritancePayment, ActivatorBase {
  ...
  */
    function getBeneficiary(ActivatorRequirements memory requirements)
        private
        view
        returns (address)
    {
        ...
        if (requirements.hasVault) {
            ...
            require(
                IWallet(beneficiary).isActiveOwner(msg.sender) == true,
                "not active owner of the vault"
            );
        }
        uint256 kiroAmount = s_whitelist[msg.sender] == 0
            ? requirements.kiroAmount
            : requirements.whitelistedKiroAmount;
    ...
    modifier rateLimit() {
        require(
            s_timestamp[msg.sender] + s_timeout <= block.timestamp,
            "too early"
        );
  ...
  */
    function updateBackupFunds(address vaultOwner, uint256 funds)
        external
        override
        onlyVaultOfOwner(vaultOwner)
    {
        (uint256 payment, uint256 refund) = _updateBackupFunds(
            vaultOwner,
            funds
        );
    ...
    function _updateBackupFunds(address vaultOwner, uint256 funds)
        private
  ...
  */
    function activateBackup(address vault) external override rateLimit {
        ...
        require(!s_requireFunds || funds > 0, "no funds");
    ...
    function migrateBackup(address vaultOwner, address from) external override {
        ...
        if (address(this) == activator) {
            ...
            uint256 balanceDelta = IERC20(s_kiro).balanceOf(address(this)) -
                curBalance;
  ...
  */
    function updateInheritanceFunds(address vaultOwner, uint256 funds)
        ...
        onlyVaultOfOwner(vaultOwner)
    {
        (uint256 payment, uint256 refund) = _updateInheritanceFunds(
            vaultOwner,
            funds
        );
        _syncFunds(payment, refund);
  ...
  */
    function activateInheritance(address vault) external override rateLimit {
        ...
        if (IHeritable(vault).activateInheritance() && funds > 0) {
  ...
  */
    ...
    ) public override onlyVaultOfOwner(vaultOwner) {
        (uint256 payment, uint256 refund) = _updateTokenInheritanceFunds(
            vaultOwner,
            tokensToAdd,
            tokensToRemove,
            fundsPerToken
    ...
    function _updateTokenInheritanceFunds(
        ...
        InheritanceFunds storage sp_inheritanceFunds = s_inheritanceFunds[
            msg.sender
        ];
        ...
        for (uint256 i = 0; i < tokensToAdd.length; i++) {
            ...
            if (fundsPerToken != curTokenFunds) {
                emit TokenInheritanceFundsUpdated(
                    vaultOwner,
                    msg.sender,
                    token,
                    UPDATED,
  ...
  */
    function activateTokenInheritance(address vault, address token)
        external
        override
        rateLimit
    {
        ...
        if (IHeritable(vault).activateTokenInheritance(token) && funds > 0) {
            IERC20(s_kiro).safeTransfer(beneficiary, funds);
    ...
    function migrateTokenInheritance(
        ...
        if (address(this) == activator) {
            ...
            for (uint256 i = 0; i < tokens.length; i++) {
                ...
                uint256 curBalance = IERC20(s_kiro).balanceOf(address(this));
                oneToken[0] = token;
                IInheritancePayment(from).migrateTokenInheritance(
                    vaultOwner,
                    from,
                    oneToken
                );
    ...
    function updateAllInheritanceFunds(
        ...
        (uint256 payment1, uint256 refund1) = _updateInheritanceFunds(
            vaultOwner,
            funds
        );
        (uint256 payment2, uint256 refund2) = _updateTokenInheritanceFunds(
            vaultOwner,
            tokensToAdd,
            tokensToRemove,
            fundsPerToken
    ...
    function updateAllFunds(
        ...
        (uint256 payment1, uint256 refund1) = _updateBackupFunds(
            vaultOwner,
            backupFunds
        );
        (uint256 payment2, uint256 refund2) = _updateInheritanceFunds(
            vaultOwner,
            inheritanceFunds
        );
        (uint256 payment3, uint256 refund3) = _updateTokenInheritanceFunds(
            vaultOwner,
            tokensToAdd,
            tokensToRemove,
            fundsPerToken
