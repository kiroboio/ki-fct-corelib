...
const types = {
    ...
    validation: constants_1.Validation,
};
class EIP712 extends FCTBase_1.FCTBase {
    ...
    getTypedDataTypes() {
        ...
        if (Number(recurrency.maxRepeats) > 1) {
            optionalTypes = (0, deepMerge_1.deepMerge)(optionalTypes, { Recurrency: EIP712.types.recurrency });
            additionalTypesInPrimary.push({ name: "recurrency", type: "Recurrency" });
        }
        if (multisig.externalSigners.length > 0) {
            optionalTypes = (0, deepMerge_1.deepMerge)(optionalTypes, { Multisig: EIP712.types.multisig });
            additionalTypesInPrimary.push({ name: "multisig", type: "Multisig" });
        }
        if (this.FCT.computed.length > 0) {
            optionalTypes = (0, deepMerge_1.deepMerge)(optionalTypes, { Computed: EIP712.types.computed });
        }
        if (this.FCT.validation.get().length > 0) {
            optionalTypes = (0, deepMerge_1.deepMerge)(optionalTypes, { Validation: EIP712.types.validation });
        }
        return {
