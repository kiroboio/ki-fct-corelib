...
class BatchMultiSigCall {
    ...
    static from = (input, messageHash) => {
        if (messageHash && messageHash.length === 66) {
            const cached = cache_1.FCTCache.get(messageHash.toLowerCase());
            if (cached)
                return cached;
        }
        ...
        FCT.importFCT(input);
        if (messageHash && messageHash.length === 66)
            cache_1.FCTCache.set(messageHash.toLowerCase(), FCT);
        return FCT;
    };
    static fromMap = (input, map, messageHash) => {
        if (messageHash && messageHash.length === 66) {
            const cached = cache_1.FCTCache.get(`map:${messageHash}`.toLowerCase());
            if (cached)
                return cached;
        }
        const FCT = new BatchMultiSigCall();
        ...
        if (messageHash && messageHash.length === 66)
            cache_1.FCTCache.set(`map:${messageHash}`.toLowerCase(), FCT);
        return FCT;
    };
