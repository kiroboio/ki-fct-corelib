...
class FetchUtility {
    ...
    constructor({ rpcUrl, chainId, provider, multicallAddress }) {
        ...
        const mcallAddress = multicallAddress || constants_1.multicallContracts[chainId];
        if (!mcallAddress) {
            throw new Error("Multicall contract not found for this chain");
        }
        this.multicallContract = new ethers_1.ethers.Contract(mcallAddress, Interfaces_1.Interfaces.Multicall, provider);
    }
