...
import { EIP712 } from "../EIP712/EIP712";
import { Options } from "../Options/Options";
import { EIP712StructTypes } from "./helpers/EIP712StructTypes";
...
export class ExportFCT {
  ...
  public getTypedDataTypes(): TypedDataTypes {
    ...
    if (Number(recurrency.maxRepeats) > 1) {
      optionalTypes = _.merge(optionalTypes, { Recurrency: EIP712.recurrency });
      additionalTypesInPrimary.push({ name: "recurrency", type: "Recurrency" });
    }

    if (multisig.externalSigners.length > 0) {
      optionalTypes = _.merge(optionalTypes, { Multisig: EIP712.multisig });
      additionalTypesInPrimary.push({ name: "multisig", type: "Multisig" });
    }

    if (this.FCT.computed.length > 0) {
      optionalTypes = _.merge(optionalTypes, { Computed: EIP712.computed });
    }

    return {
      EIP712Domain: EIP712.domain,
      Meta: EIP712.meta,
      Limits: EIP712.limits,
      ...optionalTypes,
      ...transactionTypes,
      ...structTypes,
