...
import { BatchMultiSigCallTypedData, TypedDataMessage, TypedDataTypes } from "../../types";
import { VersionBase } from "../../versions/bases/VersionBase";
...
export class EIP712 extends FCTBase {
  ...
  public getTypedDataDomain() {
    ...
    const Version = getVersionClass(this.FCT);
    const domain = Version.EIP712.getDomainData(this.FCT.chainId);
    if (!domain) {
      throw new Error(`ChainId ${this.FCT.chainId} is not supported. Please provide a custom EIP712 domain.`);
    }
    return domain;
  }
