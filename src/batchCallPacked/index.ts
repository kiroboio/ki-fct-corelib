...
const getBatchCallPackedData = async (web3: Web3, factoryProxy: Contract, call: BatchCallInputData, i: number) => {
  ...
  const hashedData = defaultAbiCoder.encode(
    ...
    [typeHash, call.to, call.value, getSessionIdERC20(i), call.data]
  );
  ...
  return {
    ...
    value: call.value,
    signer: call.signer,
    sessionId: getSessionIdERC20(i) + v.slice(2).padStart(2, "0"),
    data: call.data,
  };
};
...
export class BatchCallPacked {
  ...
  async addTx(tx: BatchCallInputData) {
    const data = await getBatchCallPackedData(this.web3, this.FactoryProxy, tx, this.calls.length);
  ...
  async addMultipleTx(txs: BatchCallInputData[]) {
    const data = await Promise.all(
      txs.map((tx, i) => getBatchCallPackedData(this.web3, this.FactoryProxy, tx, this.calls.length + (i + 1)))
    );
    this.calls = [...this.calls, ...data];
    return data;
  }
